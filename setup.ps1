...
if (Test-Path $configPath) {
    try {
        ...
        if ($ocConfig.models.providers.vllm.baseUrl) {
            ...
            Write-Host "   [OK] LLM endpoint: $llmEndpoint"
            
            # 如果 endpoint 包含 localhost → 警告
            if ($llmEndpoint -like "*localhost*") {
                Write-Host "   [WARN] 检测到 localhost → 建议改为云端地址，否则只在本地可用"
            }
        } else {
            Write-Host "   [WARN] 未检测到 LLM endpoint"
        }
        ...
        if ($ocConfig.channels.feishu.appId) {
            ...
            Write-Host "   [OK] Feishu App ID: $feishuAppId"
        }
    } catch {
        Write-Host "   [WARN] 读取配置文件失败: $($_.Exception.Message)"
    }
} else {
    Write-Host "   [WARN] 未找到 OpenClaw 配置文件: $configPath"
    Write-Host "   [WARN] 请先确保 OpenClaw 已运行"
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($userInfo.userOpenId) {
            ...
            Write-Host "   [OK] 自动检测到 Open ID: $USER_OPEN_ID"
        }
    }
} catch {
    # 忽略错误
...
if ([string]::IsNullOrEmpty($USER_OPEN_ID)) {
    ...
    if ([string]::IsNullOrEmpty($USER_OPEN_ID)) {
        ...
        Write-Host "   [WARN] 留空，使用占位符，请稍后手动修改 config.json"
    }
}
...
# Build config values to avoid inline if in hashtable
$feishuAppIdVal = "YOUR_APP_ID"
if ($feishuAppId) { $feishuAppIdVal = $feishuAppId }

$llmEndpointVal = "YOUR_LLM_ENDPOINT"
if ($llmEndpoint) { $llmEndpointVal = $llmEndpoint }

$llmModelVal = "YOUR_MODEL"
if ($llmModel) { $llmModelVal = $llmModel }

$config = @{}
$config.feishu = @{}
$config.feishu.app_id = $feishuAppIdVal
$config.feishu.app_secret = $APP_SECRET

$config.roles = @{}
$config.roles.decision_maker_open_id = $USER_OPEN_ID
$config.roles.organizer_open_id = $USER_OPEN_ID

$config.llm = @{}
$config.llm.endpoint = $llmEndpointVal
$config.llm.model = $llmModelVal
...
Write-Host "   [OK] config.json 已创建: $configJsonPath"
Write-Host ""

Write-Host "[OK] Step 1 配置初始化完成！"
...
Write-Host ""
Write-Host "   个人执行任务单"
Write-Host "     内容：谁做什么何时完成的任务"
Write-Host "     发给：每个任务的执行人"
Write-Host ""
Write-Host "   信息分发快报"
Write-Host "     内容：不需行动但需知晓的背景趋势"
...
Write-Host ""
Write-Host "归档后支持按日期或参会人或议题检索。"
Write-Host ""
Write-Host "需要：一个会议记录归档表多维表格"
Write-Host ""
...
Write-Host ""
Write-Host "  1 - 分级纪要 + 综合归档  多份文档按角色分发 + 完整内容合并"
Write-Host "  2 - 任务管理             自动录入 + 到期提醒 + 逾期升级"
Write-Host "  3 - 会前历史关联         重复矛盾遗漏检测 + 历史推送"
Write-Host "  4 - 会议记录归档         逐字稿 + 综合纪要存入表格"
Write-Host ""
...
Write-Host ""
Write-Host "   [OK] 已选模块: $($selectedModules -join ', ')"
Write-Host ""

Write-Host "[OK] Step 3 选择完成！"
...
if ($hasModuleB) {
    ...
    Write-Host "  字段：任务内容(文本) | 负责人(人员) | 截止时间(日期)"
...
if ($hasModuleD) {
    ...
    Write-Host "  字段：会议主题(文本) | 会议日期(日期)"
    Write-Host "        参会人(人员) | 逐字稿链接(文本)"
    Write-Host "        纪要整理链接(文本)"
    Write-Host "  (注意：逐字稿链接和纪要整理链接是两个独立字段！)"
    Write-Host ""
}
...
if ($tableChoice -eq "B" -or $tableChoice -eq "b") {
    Write-Host ""
    Write-Host "   正在通过飞书 API 创建多维表格..."
    Write-Host "   请确保你的飞书应用已开通 bitable:app 权限"
    Write-Host ""
    Write-Host "   目前自动化创建功能需要你手动创建多维表格，"
    Write-Host "   后续版本会支持一键自动创建。"
    Write-Host ""
    Write-Host "   请手动在飞书中创建多维表格："
    if ($hasModuleB) {
        Write-Host "   1. 创建[会议任务管理表]，包含以下字段："
        Write-Host "      - 任务内容(文本)"
        Write-Host "      - 负责人(人员)"
        Write-Host "      - 截止时间(日期)"
        Write-Host "      - 当前状态(单选:待开始/进行中/已完成/已逾期)"
    ...
    if ($hasModuleD) {
        Write-Host "   2. 创建[会议记录归档表]，包含以下字段："
        Write-Host "      - 会议主题(文本)"
        Write-Host "      - 会议日期(日期)"
        Write-Host "      - 参会人(人员)"
        Write-Host "      - 逐字稿链接(文本)"
...
if ($hasModuleB) {
    ...
    if ($taskBaseUrl -ne "") {
        if ($taskBaseUrl -match "base/([a-zA-Z0-9]+)") {
            ...
            Write-Host "   [OK] 已解析 task base token: $($matches[1])"
...
if ($hasModuleD) {
    ...
    if ($meetingBaseUrl -ne "") {
        if ($meetingBaseUrl -match "base/([a-zA-Z0-9]+)") {
            ...
            Write-Host "   [OK] 已解析 meeting base token: $($matches[1])"
...
# 自检结果
Write-Host "  飞书连接：[OK] 配置已保存（App ID: $($config.feishu.app_id)）"
if ($llmEndpoint -ne "") {
    Write-Host "  LLM 连通性：[OK] 已检测（model: $llmModel）"
} else {
    Write-Host "  LLM 连通性：[WARN] 未检测到，请稍后手动配置"
}
...
if ($hasModuleB) {
    if ($config.tables.task_management_base_token -ne "") {
        Write-Host "  会议任务管理表：[OK] 已就绪"
    } else {
        Write-Host "  会议任务管理表：[WARN] 未配置，请稍后手动添加 token"
    }
}
if ($hasModuleD) {
    if ($config.tables.meeting_records_base_token -ne "") {
        Write-Host "  会议记录归档表：[OK] 已就绪"
    } else {
        Write-Host "  会议记录归档表：[WARN] 未配置，请稍后手动添加 token"
    }
}
...
$registerCron = Read-Host "  注册吗？(Y/n)"
if ($registerCron -ne "n" -and $registerCron -ne "N") {
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  [OK] 逾期提醒 Cron 已注册"
            try {
                $cronObj = $cronResult | ConvertFrom-Json
                if ($cronObj.id) {
                    $newJobId = $cronObj.id
                    $config.cron.overdue_reminder_job_id = $newJobId
                    $config | ConvertTo-Json -Depth 10 | Set-Content $configJsonPath -Encoding UTF8
                    Write-Host "  [OK] Cron Job ID: $newJobId 已写入 config.json"
                } else {
                    Write-Host "  [WARN] 无法解析 Cron Job ID，请稍后手动检查"
                }
            } catch {
                Write-Host "  [WARN] 无法解析 Cron Job ID，请稍后手动检查"
            }
        } else {
            Write-Host "  [WARN] Cron 注册失败，可稍后手动注册"
        }
    } catch {
        Write-Host "  [WARN] Cron 注册失败（openclaw CLI 可能未就绪）"
    }
}

Write-Host ""
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
Write-Host "  [OK] 安装配置完成！（第 5 步 / 共 5 步）"
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
Write-Host ""
Write-Host "下一步："
